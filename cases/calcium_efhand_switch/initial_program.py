...
def propose_strategy():
    """OpenEvolve edits the calcium EF-hand switch protein topology/region blueprint only."""
    strategy = base_strategy()
    strategy["layout_plan"] = {
        "binder_domain_order": ['CaM_EFhand_domain'],
        "secondary_structure_priors": {'EFhand_loop_1': 'loop', 'EFhand_loop_2': 'loop', 'EFhand_loop_3': 'loop', 'EFhand_loop_4': 'loop'},
        "design_regions": [{'bind_to': ['EFhand_loop_1', 'EFhand_loop_2', 'EFhand_loop_3', 'EFhand_loop_4'],
  'disfavored_residues': ['C', 'W', 'F'],
  'favored_residues': ['D', 'E', 'N', 'Q', 'S', 'T', 'G'],
  'large_jump': True,
  'max_mutations_per_step': 6,
  'mutation_ops': {'block': 0.16,
           'motif_graft': 0.06,
           'point': 0.32,
  ...
  'position': 1,
  'priority_boost': 1.45,
  'role': 'retune acidic/polar Ca2+ pocket loops without losing EF-hand geometry',
  'secondary_structure': 'loop'},
 {'bind_to': ['CaM_scaffold_1', 'CaM_scaffold_2', 'CaM_scaffold_3', 'CaM_scaffold_4'],
  'disfavored_residues': ['C', 'P'],
  'favored_residues': ['A', 'L', 'I', 'V', 'S', 'T', 'N', 'Q', 'E', 'K'],
  'max_mutations_per_step': 1,
  'mutation_ops': {'block': 0.03, 'point': 0.92, 'swap': 0.05},
  'mutation_rate': 0.018,
  'name': 'cam_lobe_scaffold_guardrail',
  'operator_phase': 'stabilize',
  'policy_weight': 0.3,
  'position': 2,
  'priority_boost': 0.45,
  'role': 'sparse helix/scaffold support for calcium-gated peptide binding',
...
_LOCKED_RUNTIME_DEFAULTS: Dict[str, Any] = {
    ...
    "mcts_output_dir": "artifacts/calcium_efhand_switch/inner_loop_runs/latest",
    "mcts_save_tree": True,
    ...
    "external_kb_retrieval_weight": 0.50,
...
def run_search(seed: Optional[int] = None):
    return run_design_search(
        _runtime_strategy(),
        seed=seed,
        **current_case_kwargs("calcium_efhand_switch"),
    )
