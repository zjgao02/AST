...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Embed TetR InterPro fragment records with an ESM2 model."
    )
    kb_dir = project_root() / "data" / "tetr_kb"
    parser.add_argument("--records", type=Path, default=kb_dir / "tetr_interpro_records.jsonl")
    parser.add_argument("--embeddings", type=Path, default=kb_dir / "embeddings_esm2_t6_8M_tetr_interpro.npy")
    parser.add_argument("--metadata", type=Path, default=kb_dir / "embedding_metadata_esm2_t6_8M_tetr_interpro.jsonl")
    parser.add_argument("--manifest", type=Path, default=kb_dir / "embedding_manifest_esm2_t6_8M_tetr_interpro.json")
    ...
    metadata_rows = []
    with torch.no_grad():
        for start in tqdm(range(0, len(selected), args.batch_size), desc="embedding TetR InterPro"):
