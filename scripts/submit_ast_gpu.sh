...
if [[ "$SUBMIT_MODE" == "sbatch" && -n "$(command -v sbatch || true)" ]]; then
  BATCH_FILE="$LOG_DIR/${RUN_NAME}.sbatch.sh"
  {
    ...
    echo "export ASTEVOLVE_PROTENIX_MODEL_NAME='${ASTEVOLVE_PROTENIX_MODEL_NAME:-}'"
