...
def _get_scalar_metric_from_compiled(
    ...
    if chain_ids:
        key = ("scalar", metric, tuple(chain_ids))
        if key in struct_cache:
            return float(struct_cache[key])

    generic_key = ("scalar", metric, None)
    if generic_key in struct_cache:
        return float(struct_cache[generic_key])

    legacy_key = f"_{metric}"
    if legacy_key in compiled and compiled[legacy_key] is not None:
        return float(compiled[legacy_key])

    if metric == "plddt" and "_plddt" in compiled and compiled["_plddt"] is not None:
        return float(compiled["_plddt"])
...
def _maybe_fetch_residue_confidence(
    ...
    """
    尝试动态调用 protenix_api.run_protenix_confidence_multichain。
    如果用户还没更新 protenix_api.py，这里会静默跳过。
    """
    if not chain_ids:
        return

    struct_cache = _ensure_struct_cache(compiled)
    needed = [cid for cid in chain_ids if ("residue_plddt", cid) not in struct_cache]
    if not needed:
        return
    ...
    try:
        ...
        for k, v in metrics.items():
            struct_cache[("scalar", str(k), tuple(chain_ids))] = float(v)
...
class ProtenixPlddtTerm(EnergyTerm):
    """
    全局结构质量项。
    兼容旧名字 chai_plddt，但底层是 protenix。
    """
    kind: str = "chai_plddt"
    ...
    def penalty(self, seqs: Dict[str, str], compiled: Dict[str, Any]) -> float:
        ...
        cached = _get_scalar_metric_from_compiled(compiled, self.metric, chain_ids)
        if cached is not None:
            metric_val = float(cached)
        else:
            chains = []
            for cid in chain_ids:
                s = seqs.get(cid, "")
            ...
            key = (
                ...
                + f"|{self.model_name}|{self.seed}|{self.metric}|{self.mode}"
            )
            if key in self._cache:
                metric_val = self._cache[key]
            else:
                metric_val = run_protenix_plddt_multichain(
                    pred_name=_default_pred_name(chain_ids),
                    chains=chains,
                    metric=self.metric,
                    device=self.device,
                ...
                self._cache[key] = float(metric_val)

        score = float(metric_val) / float(self.scale)
...
class ProtenixPlddtDeltaTerm(EnergyTerm):
    ...
    def _metric_for(self, seqs: Dict[str, str], chain_ids: List[str]) -> Optional[float]:
        ...
        cached = _get_scalar_metric_from_compiled(None if False else {}, self.metric, chain_ids)
        # 上面这句不要删逻辑位置，下面真正从 compiled 取值在 penalty 中统一处理
        # 这里只保持接口简洁，避免外部直接调用时误用 compiled。
        if cached is not None:
            return float(cached)
        ...
        val = run_protenix_plddt_multichain(
            pred_name=_default_pred_name(chain_ids),
            chains=chains,
            metric=self.metric,
            device=self.device,
    ...
    def penalty(self, seqs: Dict[str, str], compiled: Dict[str, Any]) -> float:
        mA_cached = _get_scalar_metric_from_compiled(compiled, self.metric, self.chains_A)
        mB_cached = _get_scalar_metric_from_compiled(compiled, self.metric, self.chains_B)

        mA = mA_cached if mA_cached is not None else self._metric_for(seqs, self.chains_A)
        mB = mB_cached if mB_cached is not None else self._metric_for(seqs, self.chains_B)
        ...
        loss = torch.clamp(
            torch.tensor(self.delta_threshold) - torch.tensor(delta),
            min=0.0,
        ).item() * self.scale
...
class ResiduePlddtTerm(EnergyTerm):
    """
    新增：按 residue / segment 的 pLDDT 约束。

    用法建议：
    1. 对 binder 接口段做低置信度惩罚
    2. 用 aggregation="softmin" 或 "min" 强调最差位点
    3. 可选只看某些 positions，也可通过 segment_filter 自动抓一段

    依赖：
    - inner_opt.py / protenix_api.py 后续把逐残基 pLDDT 写入 compiled["_struct_cache"][("residue_plddt", chain_id)]
    """
    kind: str = "residue_plddt"
    chain_id: Optional[str] = None
    segment_filter: Optional[Dict[str, Any]] = None
    positions: Optional[List[int]] = None
    mode: str = "high"                 # "high" 希望高于 target；"low" 希望低于 target
    target: float = 70.0
    scale: float = 100.0
    aggregation: str = "mean"          # mean / min / max / softmin
    ...
    def _aggregate(self, vals: List[float]) -> float:
        ...
        if self.aggregation == "softmin":
            t = max(1e-6, float(self.softmin_temperature))
            w = np.exp(-arr / t)
            w = w / max(w.sum(), 1e-12)
...
_TERM_REGISTRY: Dict[str, Callable[[Dict[str, Any]], EnergyTerm]] = {
    "alphabet": lambda p: AlphabetTerm(allowed=set(p.get("allowed", AA_SET))),
    ...
    "residue_plddt": lambda p: ResiduePlddtTerm(
        ...
        positions=[int(x) for x in p.get("positions", [])] if p.get("positions") is not None else None,
        mode=str(p.get("mode", "high")),
        target=float(p.get("target", 70.0)),
        scale=float(p.get("scale", 100.0)),
...
def energy(
    ...
    return float(energy_breakdown(seqs, compiled, weighted_terms)["total"])