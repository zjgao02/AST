...
class ExternalKnowledgeProvider:
    ...
    def _load_embedding_index(self) -> None:
        ...
        embeddings_path = Path(manifest.get("embeddings", ""))
        metadata_path = Path(manifest.get("metadata", ""))
        records_path = Path(manifest.get("records", ""))
    ...
    def query_sequence(
        ...
        cache_key = f"{node_name}|{class_filter}|{k}|{seq}"
        ...
        for idx in order:
            ...
            if class_filter and row.get("substructure_class") != class_filter:
                continue
            if wanted_node and row.get("substructure_id") != wanted_node:
                continue
    ...
    def get_sequence_prior(
        ...
        if not self.retrieval_enabled or node_kind != "cdr":
            return static_prior
            ...
            "top_hits": [
                {
                    ...
                    "sequence": (hit.get("record") or {}).get("sequence_fragment"),
                }
